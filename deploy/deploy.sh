...
# Deploy Terraform
cd terraform/
terraform init -reconfigure -backend-config="bucket=$S3_STATE" -backend-config="key=combine_renew.tfstate" -backend-config="region=us-west-2" -backend-config="profile=$PROFILE"
